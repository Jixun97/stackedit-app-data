...
- **材料**：黑色虚线绘制的器件横截面图中， $AlN$压电致动器直接放置在$Si_3N_4$微环谐振器的顶部，该谐振器嵌入 5.4 μm 的 $SiO_2$包层中。ju为了施加垂直电场，将 1 μm 厚的 $AlN$ 薄膜夹在顶部 $Al$（100 nm 厚）和底部 Mo（100 nm 厚）金属层之间。 $Si_3N_4$波导采用减材工艺制造，高度为 0.8 μm，宽度为 1.8 μm。 波导完全埋入 5.4 μm 厚的 $SiO_2$ 包层中（Mo 层中的 2.4 μm SiO2 可以防止金属吸收）。 整个器件位于 230 μm 厚的硅基板上。 微环谐振器的半径为118μm，放置在盘致动器边缘3μm处。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTU3NDY1NTg5NSwtMTIyNjY0MzkwNiwzMT
